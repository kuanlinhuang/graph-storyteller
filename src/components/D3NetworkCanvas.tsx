...
export const D3NetworkCanvas = ({
  data,
  onDataChange
}: D3NetworkCanvasProps) => {
  ...
  useEffect(() => {
    ...
    defs.append('marker').attr('id', 'arrowhead').attr('viewBox', '0 -5 10 10').attr('refX', 15).attr('refY', 0).attr('markerWidth', 6).attr('markerHeight', 6).attr('orient', 'auto').append('path').attr('d', 'M0,-5L10,0L0,5').attr('fill', edgeColor[0]).style('opacity', 0.7);
    ...
    // Draw links
    const linkElements = g.selectAll<SVGLineElement, D3Link>('.link').data(links).enter().append('line').attr('class', 'link').style('stroke', edgeColor[0]).style('stroke-width', d => Math.max(1, d.weight * 2)).style('opacity', d => Math.min(1, Math.max(0.3, d.weight / 10))).attr('marker-end', showDirected ? 'url(#arrowhead)' : null);

    // Draw nodes
    const nodeElements = g.selectAll<SVGCircleElement, D3Node>('.node').data(nodes).enter().append('circle').attr('class', 'node').attr('r', d => d.radius).style('fill', d => getNodeColor(d.type)).style('stroke', 'hsl(var(--background))').style('stroke-width', 2).style('cursor', 'pointer').on('click', (event, d) => {
      setSelectedNode(d);
      toast.info(`Selected: ${d.label}`);
    }).call(d3.drag<SVGCircleElement, D3Node>().on('start', (event, d) => {
  ...
  const centerGraph = useCallback(() => {
    ...
    // Center the view
    svg.transition().duration(750).call(d3.zoom<SVGSVGElement, unknown>().transform, d3.zoomIdentity.translate(0, 0).scale(1));
  ...
  return <div className="space-y-4">
      ...
      <Card className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Slider value={forceStrength} onValueChange={setForceStrength} min={-500} max={-10} step={10} className="w-24" />
            <span className="text-xs text-muted-foreground w-8">{forceStrength[0]}</span>
          </div>
          
          <div className="flex items-center gap-2">
            <Label className="text-sm">Distance:</Label>
            <Slider value={linkDistance} onValueChange={setLinkDistance} min={20} max={200} step={10} className="w-24" />
            <span className="text-xs text-muted-foreground w-8">{linkDistance[0]}</span>
          </div>

            <div className="flex items-center gap-2">
              <Label className="text-sm">Node Size:</Label>
              <Slider value={nodeSize} onValueChange={setNodeSize} min={0} max={20} step={1} className="w-24" />
              <span className="text-xs text-muted-foreground w-8">{nodeSize[0]}</span>
            </div>

          <div className="flex items-center gap-2">
            <Label className="text-sm">Font Size:</Label>
            <Slider value={fontSize} onValueChange={setFontSize} min={8} max={24} step={1} className="w-24" />
            <span className="text-xs text-muted-foreground w-8">{fontSize[0]}</span>
          </div>
          
          <div className="flex items-center gap-2">
            <Label className="text-sm">Text Color:</Label>
            <input type="color" value={textColor[0].includes('hsl') ? '#000000' : textColor[0]} onChange={e => setTextColor([e.target.value])} className="w-8 h-8 rounded cursor-pointer" />
          </div>
          
          <div className="flex items-center gap-2">
            <Label className="text-sm">Edge Color:</Label>
            <input type="color" value={edgeColor[0].includes('hsl') ? '#3b82f6' : edgeColor[0]} onChange={e => setEdgeColor([e.target.value])} className="w-8 h-8 rounded cursor-pointer" />
          </div>
          
          <div className="flex items-center gap-2">
            <Label className="text-sm">Directed:</Label>
            <Button onClick={() => setShowDirected(!showDirected)} size="sm" variant={showDirected ? "default" : "outline"} className="h-8 w-16">
              {showDirected ? "ON" : "OFF"}
            </Button>
          </div>
        </div>
      ...
      {/* Selected Node Info */}
      {selectedNode && <Card className="p-4">
          ...
          <div className="space-y-2">
            ...
            {selectedNode.metadata && <div className="text-sm text-muted-foreground">
                {Object.entries(selectedNode.metadata).map(([key, value]) => <div key={key}>
